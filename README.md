...
A API desta branch usa o SQL Server via `ConnectionStrings__DefaultConnection`, apontando para o serviço `sqlserver`. O MongoDB também sobe no compose para facilitar a comparação com a abordagem NoSQL documentada e com a branch equivalente.
...
As migrations do EF Core continuam sendo aplicadas manualmente:
...
* **Mapeamento do banco** usa **Fluent API** no `OrderFinanceControlDbContext` para definir regras de persistência, como nomes de tabelas, chaves primárias, relacionamentos, índices únicos, tamanho de colunas, precisão decimal, conversão de enum e comportamento de delete.
...
Regras de negócio que dependem de consulta ou estado do sistema continuam nos casos de uso, como verificar se um cliente existe antes de criar um pedido, se produtos existem, ou se um pedido já foi pago. Ou seja: DTO valida formato e limites básicos, Fluent API valida o schema e os casos de uso validam comportamento de negócio.
